...
### Projekt-spezifische Timeline-Regeln (Azure GanttOps)

- `timeline-details-input` ist die Standardklasse für Timeline-Detail-Eingabefelder (Inputs/Textareas), damit Fokus-, Border- und Typografie-Verhalten konsistent bleiben.
- Die Ausrichtung in `.timeline-left-sidebar-row` darf nicht hartkodiert sein, sondern muss über eine User-Option steuerbar sein.
- Der Toggle für diese Ausrichtung gehört in `.timeline-left-sidebar-header`.
- Erlaubte Werte für `justify-content` sind ausschließlich `flex-start` und `flex-end` (Default: `flex-start`).
- Nutzerpräferenz muss persistent gespeichert werden (User-Preferences), nicht nur als flüchtiger UI-State.
